...
static char *error_list[ERROR_NUMBER] = {
    [0] = "Undefined error: 0",
...
char *s21_strerror(int errnum) {
  ...
  if (errnum >= 0 && errnum < ERROR_NUMBER) {
    error = error_list[errnum];
  } else {
    s21_strcpy(buf, UNKNOWN_ERROR);
    char *int_to_ascii = int_to_char(errnum);
    s21_strcat(buf, int_to_ascii);
    free(int_to_ascii);
    error = buf;
  }
  return error;
...
char *int_to_char(int errnum) {
  ...
  if (neg == 1) {
    c = (char *)realloc(c, s21_strlen(c) + 1);
    c[index] = '0';
  }
