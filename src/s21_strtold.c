...
/**
 * @brief Searches for exponent notation in string buffer and applies found
 * exponent to res. In case of any errors sets res to 0.
 *
...
/**
 * @brief Returns true/false if input str includes scientific notation symbols,
 * such as Inf, NAN, e
...
static long double s21_atof(const char *buffer) {
  ...
  if (*p == '.') {
    ...
    int trailing_zeros = s21_strspn(p, "0");  // s21_strspn
